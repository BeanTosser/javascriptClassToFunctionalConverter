...
class App extends React.Component {
  constructor(props) {
    ...
    this.withdrawAmountTextPrompt =
      'Enter amount to withdraw or click "Send all" to withdraw all funds';
  ...
  async createTxGenerator(wallet) {
    ...
    let daemonConnection = new MoneroRpcConnection(
      WALLET_INFO.serverUri,
  ...
  convertStringToRestoreDate(str) {
    ...
    if (str.length === 10) {
      ...
      if (dateParts.length === 3) {
        ...
        throw "Invalid date; date should contain three numbers separated by two slashes";
    }
  ...
  async restoreWallet() {
    ...
    if (!(height != NaN && height % 1 === 0 && height >= 0)) {
      ...
      try {
        ...
        height = await dateRestoreHeightWallet.getHeightByDate(
          dateParts[0],
          dateParts[1],
          dateParts[2]
        );
  ...
  async generateWallet() {
    ...
    let daemonConnection = new MoneroRpcConnection(
      ...
      WALLET_INFO.serverUsername,
      WALLET_INFO.serverPassword
    ); // TODO: factor out common daemon reference so this code is not duplicated
    let daemon = await monerojs.connectToDaemonRpc({
      server: daemonConnection,
      proxyToWorker: true
    });
  ...
  async _initMain() {
    ...
    let that = this;
    await this.txGenerator.addListener(
      new (class extends MoneroTxGeneratorListener {
        async onMessage(msg) {
          that.setState({
  ...
  async refreshMainState() {
    ...
    let armPump =
      state.transactionsGenerated > this.state.transactionsGenerated;
  ...
  async confirmAbortWalletSynchronization() {
    let doAbort = confirm(
      "All synchronization will be lost. Are you sure you wish to continue?"
    );
  ...
  setCurrentSitePage(pageName) {
    if (pageName === "/sign_out") {
      let userConfirmedSignout = confirm(
        "Are you sure you want to sign out of this wallet? If you did not record the seed phrase, you will permanently lose access to the wallet and any funds contained therin! Click 'Ok' to continue"
      );
  ...
  render() {
    ...
    if (
      this.state.walletIsSynced &&
      !(this.state.balance > 0) &&
      this.state.currentSitePage != "/deposit"
    ) {
      notificationBar = (
        <Notification_Bar
          content={
            <>
              No funds deposited &thinsp;
              <Link
        ...
        <div id="app_container">
          <Router>
            ...
            <Switch>
              <Route exact path="/">
                {this.state.currentSitePage != "/" ? (
                  ...
                  <Home
                    ...
                    confirmAbortWalletSynchronization={this.confirmAbortWalletSynchronization.bind(
                      this
                    )}
                    ...
                    createDateConversionWallet={this.createDateConversionWallet.bind(
                      this
                    )}
              ...
              <Route path="/backup">
                {this.state.currentSitePage != "/backup" ? (
                  <Redirect to={this.state.currentSitePage} />
                ) : (
                  <Save_Phrase_Page
                    omit_buttons={true}
              ...
              <Route path="/withdraw">
                {this.state.currentSitePage != "/withdraw" ? (
                  <Redirect to={this.state.currentSitePage} />
                ) : (
                  <Withdraw
                    availableBalance={this.state.availableBalance}
...
class walletListener extends MoneroWalletListener {
  constructor(callingComponent) {
    // callingComponent is "App" in this case
    super();
